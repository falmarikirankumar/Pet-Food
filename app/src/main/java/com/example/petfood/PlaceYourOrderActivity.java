...
import com.example.petfood.adapters.PlaceYourOrderAdapter;
...
public class PlaceYourOrderActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        buttonPlaceYourOrder.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                onPlaceOrderButtonClick(petsModel);
            }
        });
        ...
        switchDelivery.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton compoundButton, boolean isChecked) {
                if(isChecked){
                    ...
                    isDeliveryOn = true;
                    calculateTotalAmount(petsModel);
                }else{
        ...
        initRecyclerView(petsModel);
        calculateTotalAmount(petsModel);
    }
    private void calculateTotalAmount(PetsModel petsModel){
        float subTotalAmount=0f;
        for(Menu m : petsModel.getMenus()){
            subTotalAmount += m.getPrice() * m.getTotalInCart();
        }

        tvSubtotalAmount.setText("Rs."+String.format("%.2f", subTotalAmount));
        if(isDeliveryOn) {
            tvDeliveryChargeAmount.setText("Rs."+String.format("%.2f", petsModel.getDelivery_charge()));
            subTotalAmount += petsModel.getDelivery_charge();
        }
        tvTotalAmount.setText("Rs."+String.format("%.2f", subTotalAmount));
    }


    private void onPlaceOrderButtonClick(PetsModel petsModel){
        if(TextUtils.isEmpty(inputName.getText().toString())){
            ...
            inputCardExpiry.setError("Please enter card expriry ");
            return;
    ...
    private void initRecyclerView(PetsModel petsModel){
        cartItemsRecyclerView.setLayoutManager(new LinearLayoutManager(this));
        placeYourOrderAdapter = new PlaceYourOrderAdapter(petsModel.getMenus());
